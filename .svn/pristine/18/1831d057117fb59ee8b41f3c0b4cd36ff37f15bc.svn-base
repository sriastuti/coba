...
<script type='text/javascript'>
	...
	function download(){	
		...
		swal({
		  title: "Download?",
		  text: "Download Laporan Penjualan Obat!",
		  type: "warning",
		...
		function(isConfirm){
		  if (isConfirm) {
			...
			window.open("<?php echo base_url('logistik_farmasi/frmclaporanjual/download_penjualan')?>/"+startDate+"/"+endDate+"/"+filter);
		  } else {
                ...
                <form class="form-material m-t-40 row">
                    ...
                    <div class="form-group col-md-8 m-t-10">
                        <select name="filter" id="filter" class="form-control" style="width:100%" required="">
                            ...
                            <option value="satelit1">Satelit 1</option>
                            <option value="satelit2">Satelit 2</option>
                            <option value="satelit3">Satelit 3</option>
                        </select>
                    </div>
