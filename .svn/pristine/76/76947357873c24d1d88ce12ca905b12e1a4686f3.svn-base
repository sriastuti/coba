...
class Cpembelian extends Secure_area
{
	...
	{
		//$this->_reload();
		$this->load->view('farmasi/frmpembelian');
		//$this->load->view('iri/list_antrian', $data);
	}

	function item_search()
	{
		$suggestions = $this->master->Mmobat->get_all_obat();
	...
	//	$this->form_validation->set_rules('discount', 'lang:items_discount', 'required|integer');
	    $this->form_validation->set_rules('ppn', 'lang:items_ppn', 'required|integer');
    	$description = $this->input->post("description");
    	$serialnumber = $this->input->post("serialnumber");
		$price = $this->input->post("price");
		$quantity = $this->input->post("quantity");
	 //	$discount = $this->input->post("discount"); ganti dengan
	    $ppn = $this->input->post("ppn");
		$discountval = $this->input->post("discountval");
		$marg = $this->input->post("marg");
		if ($this->form_validation->run() != FALSE)
		{
//			$this->receiving_lib->edit_item($item_id,$description,$serialnumber,$quantity,$discount,$discountval,$price);
            $this->receiving_lib->edit_item($item_id,$description,$serialnumber,$quantity,$ppn,$marg,$discountval,$price);
		}
		else
		{
	...
	function complete()
	{
		...
		if ($this->input->post('amount_tendered'))
		{
			...
			$amount_change = $data['amount_tendered'] - round($data['total'], 2);
		...
		$data['receiving_id']='RECV '.$this->Receiving->save($data['cart'], $supplier_id,$employee_id,$comment,$payment_type,$total,$amount_tendered,$amount_change,$no_faktur);
		...
		$data['active'] = 'receivings';
		$this->load->view("farmasi/receivings/receipt",$data);
		$this->receiving_lib->clear_all(); // dari nisa
	}
