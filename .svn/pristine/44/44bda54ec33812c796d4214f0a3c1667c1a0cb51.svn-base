<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Pamlaporan extends CI_Model{
		...
		function get_lap_pemeriksaan($date0,$date1){
			...
			FROM jenis_tindakan_pa d
			LEFT JOIN pemeriksaan_patologianatomi c ON c.id_tindakan IN (d.idtindakan) 
		...
		function get_dates_detail($date0,$date1){
			...
			from pemeriksaan_patologianatomi c
			INNER JOIN jenis_tindakan_pa d ON c.id_tindakan IN (d.idtindakan)
			...
			//old SELECT distinct(LEFT(c.tgl_kunjungan,10)) as tgl_kunjungan from pemeriksaan_patologianatomi c			INNER JOIN jenis_tindakan_pa d ON c.id_tindakan IN (d.idtindakan)
		}

		function get_master_pemeriksaan_pa(){
		...
		function get_data_kunj_today(){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, a.tgl_kunjungan as tgl, count(1) as banyak, IF(a.bed='Rawat Jalan',(SELECT waktu_masuk_pa from daftar_ulang_irj where no_register=a.no_register),null) as waktu_masuk_pa,
				IF(a.bed='Rawat Jalan',(SELECT waktu_keluar_pa from daftar_ulang_irj where no_register=a.no_register),null) as waktu_keluar_pa
				FROM pemeriksaan_patologianatomi a, data_pasien b 
				WHERE a.no_medrec=b.no_medrec 
				AND left(a.tgl_kunjungan,10)  = left(now(),10) 
				GROUP BY a.no_register 
			UNION
					SELECT 'Pasien Luar' as no_cm, c.no_medrec, c.no_register, nama, c.tgl_kunjungan as tgl, count(1) as banyak, null as waktu_masuk_pa, null as waktu_keluar_pa
				FROM pemeriksaan_patologianatomi c, pasien_luar d 
		...
		function get_data_kunj_by_date($tgl){
			return $this->db->query("SELECT b.no_cm, a.no_medrec, a.no_register, nama, count(1) as banyak, IF(a.bed='Rawat Jalan',(SELECT waktu_masuk_pa from daftar_ulang_irj where no_register=a.no_register),null) as waktu_masuk_pa,
				IF(a.bed='Rawat Jalan',(SELECT waktu_keluar_pa from daftar_ulang_irj where no_register=a.no_register),null) as waktu_keluar_pa 
				FROM pemeriksaan_patologianatomi a, data_pasien b 
				WHERE a.no_medrec=b.no_medrec 
				...
				GROUP BY a.no_register 
				UNION
					SELECT 'Pasien Luar' as no_cm, c.no_medrec, c.no_register, nama, count(1) as banyak, null as waktu_masuk_pa, null as waktu_keluar_pa  
				FROM pemeriksaan_patologianatomi c, pasien_luar d 
				WHERE c.no_register=d.no_register 
		...
		function get_data_kunj_bln($bln){
			return $this->db->query("SELECT DATE_FORMAT(LEFT(tgl_kunjungan,10),'%d %M %Y') AS hari, count(*) AS jum_kunj, LEFT(tgl_kunjungan,10) as tgl 
										FROM pemeriksaan_patologianatomi
		...
		function get_total_kunj($tgl){
			return $this->db->query("SELECT COUNT(*) FROM pemeriksaan_patologianatomi WHERE left(tgl_kunjungan,10)='$tgl' GROUP BY no_register");
		}

		function get_data_tindakan(){
			return $this->db->query("SELECT a.id_tindakan, b.nmtindakan
				FROM pemeriksaan_patologianatomi a, jenis_tindakan b
		...
		function get_data_keu_tind($awal, $akhir){
			return $this->db->query("SELECT a.*, b.nama as nama FROM pendapatan_pa as a
					LEFT JOIN data_pasien as b
					ON a.no_medrec=b.no_cm
					WHERE LEFT(tgl_kunjungan , 10) >= '$awal'
					AND LEFT(tgl_kunjungan , 10) <= '$akhir'
					AND no_pa IS NOT NULL
					ORDER BY tgl_kunjungan, no_pa");
		}
