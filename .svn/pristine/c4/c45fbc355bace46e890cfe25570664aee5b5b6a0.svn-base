...
class Riclaporan extends Secure_area {
	...
	$list_pasien_obat_luar = $this->rimpasien->get_list_pasien_luar_obat($tgl_awal,$tgl_akhir,$user);
      ...
      $excel->getActiveSheet()->setCellValue('A2', "Tanggal : ".date('d F Y', strtotime($tgl_awal))." ".date('H:i',strtotime($tgl_awal))." - ".date('d F Y', strtotime($tgl_akhir))." ".date('H:i',strtotime($tgl_akhir)),PHPExcel_Cell_DataType::TYPE_STRING);
      ...
      foreach ($list_keluar_irj as $r) {
     	$total = $total + $r['vtot'] + $r['vtot_rad'] + $r['vtot_lab'] + $r['vtot_obat'] + $r['vtot_fisio'] ;
        ...
        $row++;
        $excel->getActiveSheet()->setCellValue('A'.$row, date('d-m-Y H:i',strtotime($r['tgl_cetak_kw'])),PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('B'.$row, $r['no_register'],PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('C'.$row, $r['nama'],PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('D'.$row, $jenis_bayar,PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('E'.$row, $r['vtot'] + $r['vtot_rad'] + $r['vtot_lab'] + $r['vtot_obat'] + $r['vtot_fisio'],PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('F'.$row, $diskon,PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('G'.$row, $r['vtot'] + $r['vtot_rad'] + $r['vtot_lab'] + $r['vtot_obat'] + $r['vtot_fisio']- $diskon,PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('H'.$row, $r['tunai']+$r['vtot_lab_lunas']+$r['vtot_pa_lunas']+$r['vtot_obat_lunas']+$r['vtot_fisio_lunas'],PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('I'.$row, $r['nilai_kkkd'],PHPExcel_Cell_DataType::TYPE_STRING);
        $excel->getActiveSheet()->setCellValue('J'.$row, $r['nilai_kkkd'] * $r['persen_kk'] / 100,PHPExcel_Cell_DataType::TYPE_STRING);
      }
