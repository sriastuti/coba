...
class Paclaporan extends Secure_area {
	public function __construct() {
		parent::__construct();
		$this->load->model('ird/ModelKwitansi','',TRUE);
		$this->load->model('pa/Pamlaporan','',TRUE);
	...
	public function lap_pemeriksaan(){
		$data['title'] = 'Laporan Pemeriksaan Patologi Anatomik';
		//$date0=$this->input->post('date0');
		//$date1=$this->input->post('date1');

		$this->load->view('pa/pavlappemeriksaanrange.php',$data);
	}
	...
	function showlap_pemeriksaan($date0='',$date1=''){
		...
		$hasil=$this->Pamlaporan->get_lap_pemeriksaan($date0,$date1)->result();
	...
	public function data_kunjungan()
	{
		...
		$data['title'] = 'Laporan Kunjungan Pemeriksaan Patologi Anatomi';
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
				...
				if($tampil_per=='TGL'){
					...
					$data['data_laporan_kunj']=$this->Pamlaporan->get_data_kunj_by_date($tgl)->result();
					$data['data_tindakan']=$this->Pamlaporan->get_data_tindakan_tgl($tgl)->result();
					$data['data_pemeriksaan']=$this->Pamlaporan->get_data_pemeriksaan_tgl($tgl)->result();
					$tgl1 = date('d F Y', strtotime($tgl));
					$data['date_title']="Laporan Kunjungan Pasien Patologi Anatomi <b>$tgl1</b>";
					...
					$data['data_laporan_kunj']=$this->Pamlaporan->get_data_kunj_bln($bln)->result();
					$data['data_tindakan']=$this->Pamlaporan->get_data_tindakan_bln($bln)->result();
					$data['data_pemeriksaan']=$this->Pamlaporan->get_data_pemeriksaan_bln($bln)->result();
					
					$bln1 = date('F Y', strtotime($bln));
					$bln2 = date('m', strtotime($bln));
					$bln3 = $tgl_indo->bulan($bln2);
					$data['date_title']="Laporan Kunjungan Pasien Patologi Anatomi per Hari <b>Bulan $bln3</b>";
					...
					$data['data_laporan_kunj']=$this->Pamlaporan->get_data_kunj_thn($thn)->result();
					$data['data_tindakan']=$this->Pamlaporan->get_data_tindakan_thn($thn)->result();
					$data['data_pemeriksaan']=$this->Pamlaporan->get_data_pemeriksaan_thn($thn)->result();
					
					$data['date_title']="Laporan Kunjungan Pasien Patologi Anatomi <b>Tahun $thn</b>";
					$data['pemeriksaan_title']="Laporan Pemeriksaan :";
					$data['field1']='Bulan';
			...
			$this->load->view('pa/pavlapkunjunganrange.php',$data);
		}else{
			$data['data_laporan_kunj']=$this->Pamlaporan->get_data_kunj_today()->result();
			$data['data_tindakan']=$this->Pamlaporan->get_data_tindakan()->result();
			$data['data_pemeriksaan']=$this->Pamlaporan->get_data_pemeriksaan()->result();
			$data['date_title']='Laporan Kunjungan Pasien Patologi Anatomi <b>'.date("d F Y").'</b>';
			$data['tgl']=date("Y-m-d");
			$data['field1']='No. Medrec';
			$data['tampil_per']='TGL';		
	...
	{
		$data['title'] = 'Laporan Pendapatan Penunjang Patologi Anatomi';				
		...
		$this->load->view('pa/pavpendapatan',$data);
	}

	public function lap_keu($tampil_per='',$param1='',$param2='')
	{
		$data['title'] = 'Laporan Keuangan Patologi Anatomi';
		...
		$konten="<table>
					...
					<tr>
						<td colspan=\"3\"><p align=\"center\"><br><b>Laporan Keuangan Patologi Anatomi</b></p></td>
					</tr>
					<tr>
						<td></td>
					</tr>";
		...
		if($tampil_per=='TGL'){	
			if($param1!=''){
				...
				$data_laporan_keu=$this->Pamlaporan->get_data_keu_tind_tgl($tgl)->result();
				$data_keuangan=$this->Pamlaporan->get_data_keuangan_tgl($tgl)->result();
			...
			if($param1!=''){
				...
				//$data_laporan_keu=$this->Pamlaporan->get_data_keu_tind_bln($bln)->result();
				if($param2!=''){
					$data_periode=$this->Pamlaporan->get_data_periode_bln_bycarabayar($bln, $param2)->result();
					$data_keuangan=$this->Pamlaporan->get_data_keuangan_bln_bycarabayar($bln, $param2)->result();
				} else {
					$data_periode=$this->Pamlaporan->get_data_periode_bln($bln)->result();
					$data_keuangan=$this->Pamlaporan->get_data_keuangan_bln($bln)->result();
				}
				...
				$konten=$konten."
						...
						foreach($data_periode as $row){
							//$vtot=$vtot+$row->total;
							if($param2!=''){
								$rwspn=count($this->Pamlaporan->row_table_pertgl_bycarabayar($row->tgl, $param2)->result());
							} else {
								$rwspn=count($this->Pamlaporan->row_table_pertgl($row->tgl)->result());
							}
			...
			if($param1!=''){
				...
				//$data_laporan_keu=$this->Pamlaporan->get_data_keu_tind_thn($thn)->result();
				if($param2!=''){
					$data_periode=$this->Pamlaporan->get_data_periode_thn_bycarabayar($thn, $param2)->result();
					$data_keuangan=$this->Pamlaporan->get_data_keuangan_thn_bycarabayar($thn, $param2)->result();
				} else {
					$data_periode=$this->Pamlaporan->get_data_periode_thn($thn)->result();
					$data_keuangan=$this->Pamlaporan->get_data_keuangan_thn($thn)->result();
				}
				...
				$konten=$konten."
						...
						foreach($data_periode as $row){
							//$vtot=$vtot+$row->total;
							if($param2!=''){
								$rwspn=count($this->Pamlaporan->row_table_perbln_bycarabayar($row->bln, $param2)->result());
							} else {
								$rwspn=count($this->Pamlaporan->row_table_perbln($row->bln)->result());
							}
							$rwspn1=$rwspn+1;
							$j=1;
							$vtottotal=0;
	...
	public function export_excel($tampil_per='',$param1='',$param2='')
	{
		...
		if($tampil_per=='TGL'){	
			if($param1!=''){
				...
				header('Content-Disposition: attachment;filename="Lap_Keu_Patologi_Anatomi_TGL.xlsx"');  
			...
			if($param1!=''){
				...
				header('Content-Disposition: attachment;filename="Lap_Keu_Patologi_Anatomi_Bulan.xlsx"');  
			}
			else{
				redirect('pa/Paclaporan/data_pendapatan','refresh');
			}
			...
			if($param1!=''){
				...
				$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, '-');
				$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, $vtot);
				header('Content-Disposition: attachment;filename="Lap_Keu_Patologi_Anatomi_Tahun.xlsx"'); 
			}else{
				redirect('pa/Paclaporan/data_pendapatan','refresh');
			}
		}
	...
	public function excel_lappemeriksaan($date0='',$date1=''){
		...
		$objPHPExcel->getProperties()->setCreator($namars)  
		        ...
		        ->setTitle("Laporan PA ".$namars)  
		        ->setSubject("Laporan PA ".$namars." Document")  
		        ->setDescription("Laporan PA ".$namars.", generated by HMIS.")  
		        ->setKeywords($namars)  
		        ->setCategory("Laporan PA");  
		...
		$hasil=$this->Pamlaporan->get_lap_pemeriksaan_detail($date0,$date1)->result();
		$listtgl = $this->Pamlaporan->get_dates_detail($date0,$date1)->result();
		$master_lab=$this->Pamlaporan->get_master_pemeriksaan_pa()->result();
...
//break;
		...
		header('Content-Disposition: attachment;filename="Lap_Range_PA.xlsx"');
