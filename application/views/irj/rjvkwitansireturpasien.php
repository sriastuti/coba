...
<script type='text/javascript'>
	$(function() {
		...
		var nilairetur = "<?php echo $data_pasien->vtot_retur;?>";
		if(nilairetur!='' && parseInt(nilairetur)!=0){
			$('#biaya_retur').val(nilairetur);
			$('#retur_hide').val(nilairetur);
		}

		var noteretur = "<?php echo $data_pasien->note_retur;?>";
		if(noteretur!='' && parseInt(noteretur)!=0){
			$('#note_retur').val(noteretur);
			$('#note_hide').val(noteretur);
		}
		
	});
var count = 0;
var idretur="";
function countRetur(checkbox,vtot,id){
	if(idretur==""){
		idretur = id; 
	}else{
		idretur = idretur + "," + id;
	}
	$('#idtindakanretur_hide').val(idretur);
	if(checkbox.checked == true){
        count = parseInt(count) + parseInt(vtot); 
    }else{
    	if(count>0){
    		count = parseInt(count) - parseInt(vtot);
    	}
   }
   $('#biaya_retur').val(count);
   console.log(count);
}
</script>
	...
	<div class="container-fluid">
		...
		<section class="content">
			<div class="row">
				<div class="card card-outline-info">
					<div class="card-header text-white" align="center">Rincian Biaya</div>
			...
			<?php }?>
							  ...
							  <tbody>
								...
								?>
									...
									<tr>
									  ...
									  <td><input type='checkbox' name='checkRetur[]' value='<?php echo $row->id_pelayanan_poli;?>' onchange='countRetur(this,"<?php echo $row->vtot;?>","<?php echo $row->id_pelayanan_poli;?>")' id='<?php echo $row->id_pelayanan_poli;?>' <?php if($row->retur=='1'){echo 'disabled';}?>/><label for='<?php echo $row->id_pelayanan_poli;?>' ><?php if($row->retur=='1'){echo 'Sudah Diretur';}?></label></td>
									</tr>
									<?php } } ?>
						...
						<hr>
						<h4 align="center"><b>R E T U R</b></h4>
						<hr>
		...
		<tbody>
			...
			<tr>
				...
				<td style="width:40%;">
					<div class="input-group">
						...
						<input type="text" class="form-control" name="biaya_t" id="biaya_t" placeholder="Nominal Tunai" required readonly="true">
					</div>
				</td>
				<td style="width:40%;"></td>
			</tr>
			<tr>
				<th style="width:20%;">Nominal Retur</th>
				<td>:</td>
				<td style="width:40%;">
					<div class="input-group">
						<span class="input-group-addon">Rp</span>
						<input type="text" class="form-control" name="biaya_retur" id="biaya_retur" placeholder="Nominal Retur" required readonly="true">
					</div>
				</td>
				<td style="width:40%;"></td>
			</tr>
			...
			<tr>
				<th style="width:20%;">Catatan Retur</th>
				<td>:</td>
				<td colspan="2" style="width:40%;">
					<input type="text" class="form-control" name="note_retur" id="note_retur" placeholder="Catatan Retur Pembayaran" required>
				</td>
			...
			<tr>
				...
				<td><div class="input-group">
						...
						<span class="input-group-btn">
						<button type="btn" class="btn btn-primary" onclick="setTotalAfterRetur()">Hitung</button>
					</span>
					</div>
				</td>
			</tr>
	...
	<table style="margin-left:10px;">
		<tbody>
			<tr>
				...
				<td style="width:40%;">
					<input type="text" class="form-control" name="pasien_retur" id="pasien_retur" onchange="penyetorDetail(this.value)" >
				</td>
			</tr>			
	...
	<br>
		<div class="form-inline row" align="left" style="margin-left:10px;">
			<div class="input-group">				
			<?php echo form_open('irj/rjckwitansi/st_cetak_kwitansi_retur_kt');?>
					<input type="hidden" class="form-control" name="penyetor_hide" id="penyetor_hide">
					<input type="hidden" class="form-control" name="retur_hide" id="retur_hide">
					<input type="hidden" class="form-control" name="note_hide" id="note_hide">
					<input type="hidden" class="form-control" value="<?php echo sizeof($data_tindakan);?>" name="tot_tind_hide" id="tot_tind_hide">
					<input type="hidden" class="form-control" name="idtindakanretur_hide" id="idtindakanretur_hide">
					...
					<input type="hidden" class="form-control" name="pasien_retur" id="pasien_retur_hide_1">
					...
					<span>
						<button type="submit" id="btn-cetak" class="btn btn-primary">Simpan</button>
					</span>									
			<?php echo form_close();?>
			</div>
			<!--<div class="input-group">			
...
function setTotalAfterRetur(){
	var total1 = "<?php echo $jumlah_vtot; ?>";
	var retur = $('#biaya_retur').val();
	var noteretur = $('#note_retur').val();
	//console.log(retur);
	if(parseInt(retur)>0){
		var totalend = parseInt(total1)-parseInt(retur);
			$('#totfinal').val(totalend); 
			$('#totfinal_hide_1').val(totalend);
			$('#totakhir_hide').val(total1);
			$('#retur_hide').val(retur);
			$('#note_hide').val(note_retur);
